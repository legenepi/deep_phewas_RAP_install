...
task creating_concepts {
	...
	String? code_list_folder_override = if defined(code_list) then "concept_codes" else code_list

	command <<<
		[ -n "~{code_list}" ] && unzip -d ~{code_list_folder_override} ~{code_list}
		Rscript `Rscript -e 'cat(system.file("extdata/scripts/phenotype_generation","03c_creating_concepts.R", package = "DeepPheWAS"))'` \
			...
			--health_data ~{health_data} \
			~{"--PheWAS_manifest_overide " + phewas_manifest } \
			~{"--code_list_folder_override " + code_list_folder_override}
	>>>
...
task primary_care_quantitative_phenotypes {
	...
	String? code_list_folder_override = if defined(code_list) then "PQP_codes" else code_list
	Int cores = 16
	...
	command <<<
		...
		--phenotype_save_file ~{phenotype_save_file} \
		~{"--PheWAS_manifest_overide " + phewas_manifest} \
		~{"--code_list_overide " + code_list_folder_override} \
		--N_cores ~{cores}
	>>>
